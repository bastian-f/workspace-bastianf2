...
require(["jquery","bootstrap","amplify","google"], function() {
    ...
    if (debug) console.log("ChiliPeppr Run-level 1: inside app.js define method...");
    ...
    chilipeppr = new function() {
        ...
        this.load = function(id, url) {
            ...
            if (isAppend)
                if (debug) console.log("Attempting to load URL:", url, "and prepend to start of body");
            else
                if (debug) console.log("Attempting to load " + id + " at URL " + url);

            // make sure this id doesn't already exist
            for (var i = 0; i < this.panels.length; i++) {
                if (this.panels[i].id == id) {
                    if (debug) console.log("This panel " + id + " already exists. Still giving you a callback though.");
                    if (callback) {
            ...
            if (window.location.origin.indexOf("fiddle.jshell.net") != -1 ||
                ...
                if (debug) console.log("We are doing a raw load (not via chilipeppr.com/geturl) cuz in jsfiddle, cloud9, or localhost. window.location.origin:", window.location.origin);
                $.get("//i2dcui.appspot.com/slingshot?url=" + panel.url, function(data) {
                    ...
                    if (isAppend)
                        ...
                        $(panel.id).html(data);
                    if (debug) console.log("Load of " + panel.id + " was performed via local jsfiddle/cloud9.");
                    panel.isLoaded = true;
                    if (callback) {
                        //console.log("You asked for a callback. Here it is.");
                        callback.apply();
                ...
                if (debug) console.log("We are NOT doing a raw load (via chilipeppr.com/geturl) cuz NOT in jsfiddle, cloud9, or localhost. window.location.origin:", window.location.origin);

                // see if we need to memcache bust
                var bust = "";
                if (window.location.href.indexOf("forcerefresh=true") != -1) {
                    // being asked to bust memcache
                    if (debug) console.log("Being asked to bust memcache cuz forcerefresh=true in window url");
                ...
                $.get("//i2dcui.appspot.com/geturl?" + bust + "url=" + panel.url, function(data) {
                    ...
                    if (isAppend)
                        ...
                        $(panel.id).html(data);
                    if (debug) console.log("Load of " + panel.id + " was performed.");
                    panel.isLoaded = true;
                    if (callback) {
                        //console.log("You asked for a callback. Here it is.");
                        callback.apply();
        ...
        this.subscribe = function( topic, context, callback, priority ) {
            if (debug) console.log("Got a subscribe call. topic:", topic);
        ...
        this.publish = function( topic, data ) {
            var args = Array.prototype.splice.call(arguments, 0);
            if (debug) console.log("Got a publish call. topic:", topic, " data:", data);
            amplify.publish.apply( null, args );
        };

        this.unsubscribe = function( topic, context, callback ) {
            if (debug) console.log("Got an unsubscribe call. topic:", topic);
            var args = Array.prototype.splice.call(arguments, 0);
